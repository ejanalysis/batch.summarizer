#'###################################################################################
# *** To specify a local large custom file, uncoment one of
# the following lines: mydemofile <-
# 'path/hugebatchoutput.csv' # for example mydemofile <-
# 'ejtest2.csv'
#'###################################################################################

#'##############
# *** neuro-related fields may need to be removed from these
# in batch summarizer global.R: names.e.batch
# names.e.friendly names.ej.batch names.ej.friendly

#'###################################################################################
# Get packages and source code to get functions needed ***
# These could be put into R folder and loading
# batch.summarizer package would make them available.
#'###################################################################################

require(batch.summarizer)
source("batch.summarize.R")  # will be  as package but had trouble with export tag bug

# FUNCTIONS DEFINED HERE (Defined just in this
...
#'###################################################################################
# *** These (newer versions) are also found in the packages
# called analyze.stuff and ejanalysis, (and ejscreen pkg
# provides popupunits) available at
# http://ejanalysis.github.io Could use analyze stuff to
# replace all the copies of key functions that are also here
# require(analyze.stuff) analyze.stuff::required.packages()
...
# source('pct.below.R') # returns percent of rows (or wtd
# people) that have value below specified cutoff (or mean as
# default), for each column of data.frame
...
# *** could get this from proxistat package, or keep here and
# could just use lazy loading?  - see data(counties,
# package='proxistat')
data(counties)
...
#'###################################################################################
#'# OTHER PACKAGES USED:
# library(data.table) # used by analyze.stuff or newer
# version of wtd.colMeans()
...
# MAPS PACKAGES TO OBTAIN THE leaflet PACKAGE (IF VERSION NOT
# ON CRAN?): library(devtools);
# devtools::install_github('rstudio/leaflet')
library(leaflet)  # for interactive maps
# library(leafletR) # for interactive maps? (a different
# package than leaflet)
...
#'###################################################################################
# DEFINE GLOBAL VARIABLES FOR USE IN server or ui - default
# values, etc.
#'###################################################################################

# **** MUST UPDATE ACS DATA IN CODE WHEN SWITCHING TO NEW ACS
# DATASET!

# *** COULD SPECIFY LARGE FILE THAT IS OUTPUT OF BATCH RUN
# AND INPUT TO THIS SUMMARIZER: *** Could do so here if huge
# file too large for Shiny's file upload limit - For large
# file, edit code and run a local copy of this summarizer.
...
if (!exists("mydemofile")) {
  mydemofile <- "Export_Output_Example2.csv"
}  # should update to 2016 format
if (!exists("mydemofile.pop")) {
  mydemofile.pop <- "Export_Output_Example2.pop.csv"
...
# 'map_batch_to_friendly_fieldnames_2016.csv' batchfields2016
# <- c( 'OBJECTID', 'FACID', 'LAT','LONG',
...
# ** Some or all of the map between versions of field names
# could be handled using the ejscreen package now, via
# data('ejscreenformulas'), with e.g., gdbfieldname 'CANCER'
# Rfieldname 'cancer' acsfieldname NA type 'Environmental'
...
batchfields2015 <- c("OBJECTID", "FACID", "NAME", "LAT", "LON", 
...
# ####################### FIELD NAMES FOR 2018 DATASET AND
# TOOL
...
batchfields2018 <- c("OBJECTID", "facid", "name", "lat", "lon", 
...
# This is also available via data(popupunits,
# package='ejscreen') # except this package has to be
# obtained from github for that.
popupunits <- structure(list(evar = c("pm", "o3", "cancer", "resp", 
  "dpm", "pctpre1960", "traffic.score", "proximity.npl", "proximity.rmp", 
  "proximity.tsdf", "proximity.npdes"), units = c("ug/m3", 
  ...
  "facilities/km distance", "facilities/km distance")), .Names = c("evar", 
...
# Allow user to specify user-specified # of groups of
...
threshgroup.default <- list("EJ US pctiles", "EJ Region pctiles", 
  "EJ State pctiles")
...
########################################################################################## DEMOGRAPHIC US TOTALS/US OVERALL PERCENTS -- *** MUST BE
########################################################################################## UPDATED WHEN NEW ACS DATA IS USED !
...
us.percents.2016 <- 100 * sapply(paste(names.d.batch, ".US", 
  sep = ""), get)
...
us.counts.names <- paste(gsub("pct", "", names.d.batch), ".US", 
  sep = "")
